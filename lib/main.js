...
function WikipathwaysApiClient(args) {
  ...
  function getPathway(args, responseType, stream) {
    ...
    return utils.respondUniversal(pathway, responseType);
  }

  function PathwayStream(stream) {
    //*
    console.log('stream');
    console.log(stream);
    //*/
    ...
    //var newStream = process.stdout;
    var newStream = highland([]);

    //*
    //RxNode.fromReadableStream(process.stdin)
    RxNode.fromReadableStream(stream)
      .subscribe(function(argsItem) {
        console.log('argsItem');
        console.log(argsItem);
        //return getPathway(argsItem, 'stream', process.stdout);
        return getPathway(argsItem, 'stream', newStream);
      });
      //*/
