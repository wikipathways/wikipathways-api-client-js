...
function WikipathwaysApiClient(args) {
  ...
  if (isBrowser) {
    ...
    if (!window.wikipathwaysUsername) {
      console.log('No wikipathwaysUsername available!');
    }
  }
  ...
  function getPathway(args, responseType) {
    args = args || {};
    ...
    var pathwaySource = RxNode.fromUnpausableStream(
        hyperquest(iri.toString())
    )
    .streamThrough(JSONStream.parse())
    .map(function(response) {
      var data = atob(response.data).toString();
      return data;
    });

    // TODO use panWrap from rx-extra
  ...
  function listPathways(args, responseType) {
    args = args || {};
    ...
    // TODO is actually is pausable
    var pathwayList = RxNode.fromUnpausableStream(
        hyperquest(iri.toString())
    )
    .streamThrough(JSONStream.parse('pathways.*'))
    .map(function(data) {
      var pathway = {
        '@context': [
  ...
  function login(args, responseType, stream, callback) {

    // STEP1 - login to wikipathways
    console.log('Calling: ' + baseIri);

    var postBody = {};

    loginIri = URI(baseIri).filename('login');
    loginIri.setQuery({name: args.username});
    loginIri.setQuery({pass: args.password});
    loginIri.setQuery({format: 'json'});

    loginString = loginIri.toString();
    console.log(loginString);

    var loginReq = hyperquest.get(loginString);
    var loginData = '';

    loginReq.on('data', function(buf) {loginData += buf;});
    loginReq.on('end', function() {

      console.log('data from login');
      console.log(loginData);

      var jsonLogin = JSON.parse(loginData);

      if (!jsonLogin.hasOwnProperty('auth')) {
        console.log('ooops... login problem');
      } else {
        console.log(jsonLogin.auth);
        return callback(jsonLogin.auth);
      }
  ...
  function updatePathway(args, responseType) {
    args = args || {};
    ...
    login(args, responseType, function(loginResult) {
      ...
      //console.log("revision "+args.identifier);
      ...
      //      iri.setQuery({auth: loginResult});
      //     iri.setQuery({description: args.description});
      //   }
      console.log(iri.toString());
      console.log(convertObjectToQueryString(postBody));
    ...
    //comentend
    // TODO use panWrap from rx-extra
    //return utils.respondUniversal(responseSource, responseType, stream);
